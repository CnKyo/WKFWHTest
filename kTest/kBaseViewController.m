...
@interface kBaseViewController ()<UIGestureRecognizerDelegate>
...
{
    __block kBaseViewController *vc = self;
...
{
    self.navBar.rightBtn.frame = CGRectMake(DEVICE_Width-size+20, 27, size, 31);
...
-(void)pushViewController:(UIViewController *)vc{
    if( [vc isKindOfClass:[kBaseViewController class] ] )
    {
        if( ((kBaseViewController*)vc).isMustLogin )
        {
...
-(void)gotoLoginVC
{
    ...
    FWHLoginViewController *viewController = [storyboard instantiateViewControllerWithIdentifier:@"login"];
